...
class OlmoeDecoderLayer(nn.Module):
    def __init__(self, config: OlmoeConfig, layer_idx: int):
        ...
        if config.use_ce:
            self.mlp = OlmoeSparseCompressedMoeBlock(config)
        else:
...
class OlmoeCEMLP(nn.Module):
    ...
    def forward(self, x, e1, e2, e3):
        up_output = self.act_fn(self.gate_proj(x * e1)) * self.up_proj(x * e3)
        down_output = self.down_proj(up_output * e2)
        return down_output.view(-1, self.hidden_size)
    

class OlmoeSparseCompressedMoeBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_experts = config.num_experts
        self.top_k = config.num_experts_per_tok
        self.n_ce = config.n_ce
        self.n_main_experts = self.top_k - self.n_ce 
        self.norm_topk_prob = config.norm_topk_prob
        self.gate = nn.Linear(config.hidden_size, self.num_experts, bias=False)
        self.experts = nn.ModuleList([OlmoeCEMLP(config) for _ in range(self.num_experts)])
        self.intermediate_size = config.intermediate_size
        
        # define compressed experts for each matrix in the expert
        self.ces = nn.Parameter(torch.ones(self.num_experts, 3, config.hidden_size))
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # One hot encode the selected experts to create an expert mask
        # this will be used to easily index which expert is going to be selected
        expert_mask = torch.nn.functional.one_hot(selected_experts[:, :self.n_main_experts], num_classes=self.num_experts).permute(2, 1, 0)
        ...
        for expert_idx in range(self.num_experts):
            ...
            # select the compressed experts based on sorted routing weights
            embed_idx = selected_experts[top_x][:,-self.n_ce:]
            selected_ce = self.ces[embed_idx]
            
            # aggregate the compressed experts corresponding to auxiliary experts
            routing_weights_expanded = routing_weights[top_x_list, self.n_main_experts:self.top_k]
            normalizer = routing_weights[top_x_list, self.n_main_experts:self.top_k, None].sum(dim=1)
            normalized_weights = (routing_weights_expanded / normalizer).unsqueeze(-1).unsqueeze(-1)
            normalized_ce = (selected_ce * normalized_weights).sum(dim=1)
            ce1 = normalized_ce[:, 0, :]
            ce2 = normalized_ce[:, 1, :self.intermediate_size]
            ce3 = normalized_ce[:, 2, :]

            # forward passes only through the main experts
            current_state = hidden_states[None, top_x].reshape(-1, hidden_dim)
            current_hidden_states = expert_layer(current_state, ce1, ce2, ce3) * routing_weights[top_x, idx, None]
