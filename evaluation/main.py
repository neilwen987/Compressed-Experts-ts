...
def main():
    ...
    list_data_dict = load_jsonl('/home/ubuntu/tiansheng/Compressed-Experts-ts/evaluation/GSM8K-eval/data/gsm8k_demo.jsonl', instruction="question", output="answer")
    ...
    # 预处理所有prompts - 避免在推理循环中重复构建
    print("预处理所有prompts...")
    all_prompts = []
    for sample in tqdm(list_data_dict, desc="构建prompts"):
        prompt = build_prompt(sample["instruction"], N_SHOT, COT_FLAG)
        all_prompts.append(prompt)
    
    answers = []
    batch_size = args.batch_size
    for i in tqdm(range(0, len(list_data_dict), batch_size)):
        batch_samples = list_data_dict[i : i + batch_size]
        input_texts = all_prompts[i : i + batch_size]
        generate_kwargs = dict(max_new_tokens=256, top_p=0.95, temperature=0.8,use_cache=True)
        model_completions = generate(model, tokenizer, input_texts, generate_kwargs)

        if not isinstance(model_completions, list):
            model_completions = [model_completions]

        for j, sample in enumerate(batch_samples):
            model_answer = clean_answer(model_completions[j])
            ...
            #     f"Model Completion: {model_completions[j]}\n\n"
            #     f"Is correct: {is_cor}\n\n"
            # )
        if (i // batch_size) % 10 == 0:  # 每10个batch打印一次
            print(
                f"Num of total question: {len(answers)}/{len(list_data_dict)}, "
                f"Correct num: {sum(answers)}, "
                f"Accuracy: {float(sum(answers))/len(answers):.4f}"
            )
        # print(
